#! /bin/bash
PYTHONSRCPATH=src/main.py
EPOCH_NUM=1000
b_p=pitching #should be either batting or pitching
start_year=2015
end_year=2023
model_directory=models
data_mode=csv #should be either csv or pyb or api
use_wandb=false # should be either true or false

echo $b_p"_data_"$start_year"_"$end_year

if [ "$use_wandb" = true ] ; then
    python3 $PYTHONSRCPATH -m  $b_p -y $start_year"_"$end_year -dm $data_mode -i AVG HR RAR OPS wRC+ -o WAR -e $EPOCH_NUM -sm $model_directory -w
else
    python3 $PYTHONSRCPATH -m  $b_p -y $start_year"_"$end_year -dm $data_mode -i WHIP BABIP AVG -o WAR -e $EPOCH_NUM -sm $model_directory
fi
